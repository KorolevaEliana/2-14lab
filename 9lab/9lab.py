...
def first():
    ...
    for i in range(1, 6):
        orig = Image.open(Path(f"9lab_{i}.jpg"))
        filt = orig.filter(ImageFilter.Kernel((3, 3), (-1, -1, -1, -1, 12, -1, -1, -1, -1), 1, 0))
        filt.save(Path(Path.home()) / "newphotos" / f"9lab_filter{i}.jpg")
...
def second():
    ...
    for i in range(1, 6):
        orig = Image.open(Path(f"9lab_{i}.jpg"))
        if (Path(f"9lab_{i}.jpg").suffix == '.jpg') or (Path(f"9lab_{i}.jpg").suffix == '.png'):
            filt = orig.filter(ImageFilter.Kernel((3, 3), (-1, -1, -1, -1, 12, -1, -1, -1, -1), 1, 0))
            filt.save(Path(Path.home()) / "newphotos" / f"9lab_filter{i}.jpg")
        else:
            print(f"9lab_{i} file: incorrect file type")
